...
class QueryDb {
    ...
    fun areas(): List<QueryDbArea> =
            areaRecords.values.sortedBy { it.area.id.lowercase() }.toList()

    fun registerArea(area: Area): QueryDbArea =
            areaRecords.computeIfAbsent(area.id) {
                QueryDbArea(area)
            }

    fun rooms(): List<QueryDbRoom> =
            roomRecords.values.sortedBy { it.room.vnum }.toList()

    fun registerRoom(room: Room, area: Area): QueryDbRoom =
            roomRecords.computeIfAbsent(room.vnum) {
                QueryDbRoom(room, area)
            }

    fun items(): List<QueryDbItem> =
            itemRecords.values.sortedBy { it.item.vnum }.toList()

    fun registerItem(item: Item): QueryDbItem =
            itemRecords.computeIfAbsent(item.vnum) {
                QueryDbItem(item)
            }

    fun mobiles(): List<QueryDbMobile> =
            mobileRecords.values.sortedBy { it.mobile.vnum }.toList()
...
data class QueryDbArea(
        val area: Area,
)

data class QueryDbRoom(
        val room: Room,
        val area: Area,
)
